...
for video in range(0,50):
    ...
    train_video_filespath.remove(''dfdc_train_part_%d/metadata.json'%video)
    ...
    for i in tqdm(range(len(train_video_filespath))):
        ...
        v_cap.release()
    np.save("features_%d.npy" % video,features) 
    np.save("labels_%d.npy" % video,labels)
